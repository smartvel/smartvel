...
/***/ function(module, exports, __webpack_require__) {
	...
	angular.module('smartvel-calendar', ['ngAnimate', 'ngSanitize', 'ngTouch', 'smartvel-common', 'matchmedia-ng']);
...
/***/ function(module, exports, __webpack_require__) {
	...
	angular.module('smartvel-calendar').directive('smtCalendar', function(filters, Session, locale, taxonomies) {
	  return {
	    ...
	    controller: function($scope, $compile, $filter, $element, $attrs, matchmedia, params, replicas) {
	      $scope._params = params.getParams($attrs);
	      $scope.reloadDateLocale = function() {
	        return $scope.current = $scope.current.locale(locale.currentLang().locale);
	      };
	      $scope.init = function() {
	        var forced_filters;
	        $scope.current = $scope._getCalendarStarts($scope.startsDate);
	        $scope.current_day = moment().startOf('day');
	        forced_filters = {
	        ...
	        $scope.reloadDateLocale();
	        $scope.first = moment($scope.current).clone().startOf('month').startOf('isoWeek');
	        $scope.last = moment($scope.current).clone().endOf('month').endOf('isoWeek');
	        $scope.visible_days = {};
	        $scope.days = {};
	        $scope.selected_day = void 0;
	        return $scope.details_displayed = false;
	      };
	      $scope.next_month = function() {
	        $scope.current.add(1, 'month').startOf('month');
	        return $scope.reload();
	      };
	      $scope.prev_month = function() {
	        $scope.current.subtract(1, 'month').startOf('month');
	        return $scope.reload();
	      };
	      $scope.isInCurrentMonth = function(day) {
	        return day.m.isSame($scope.current, 'month');
	      };
	      $scope.isPastDay = function(day) {
	        return day.m.isBefore($scope.current_day);
	      };
	      $scope.hasEvents = function(day) {
	        return _.size(day.events) > 0;
	      };
	      $scope.isDisabledDay = function(day) {
	        return $scope.isInCurrentMonth(day) && ($scope.isPastDay(day) || !$scope.hasEvents(day));
	      };
	      $scope.show_events = function(day) {
	        var childScope, placeHolderId;
	        if (!$scope.isDisabledDay(day)) {
	          $scope.close_details();
	          if (day.m.format('YYYY-MM-DD') !== $scope.selected_day) {
	            $scope.remove_details_panel();
	            $scope.selected_day = day.m.format('YYYY-MM-DD');
	            $scope.visible_days[$scope.selected_day].selected = true;
	            childScope = $scope.$new();
	            childScope.events = day.events;
	            if (matchmedia.isPhone()) {
	              placeHolderId = "day-" + (day.m.clone().format('YYYY-DD-MM'));
	            } else {
	              placeHolderId = "day-" + (day.m.clone().endOf('isoWeek').format('YYYY-DD-MM'));
	            }
	            angular.element(document.getElementById(placeHolderId)).after($compile("<smt-event-day-list></smt-event-day-list>")(childScope));
	            $scope.details_displayed = true;
	          } else {
	            $scope.selected_day = void 0;
	          }
	        }
	      ...
	      $scope.close_details = function() {
	        $scope.details_displayed = false;
	        if ($scope.selected_day && $scope.visible_days[$scope.selected_day]) {
	          return $scope.visible_days[$scope.selected_day].selected = false;
	        }
	      };
	      $scope.remove_details_panel = function() {
	        var old_details_panel;
	        old_details_panel = document.getElementById("calendar-details");
	        if (old_details_panel) {
	          return old_details_panel.remove();
	        }
	      };
	      $scope.createCalendar = function() {
	        var day, results;
	        $scope.reloadDateLocale();
	        $scope.week_days = (function() {
	          var i, results;
	          results = [];
	          for (day = i = 1; i <= 7; day = ++i) {
	            results.push($scope.current.isoWeekday(day).format("dddd"));
	          }
	          return results;
	        })();
	        $scope.days = {};
	        $scope.first = moment($scope.current).clone().startOf('month').startOf('isoWeek');
	        $scope.last = moment($scope.current).clone().endOf('month').endOf('isoWeek');
	        day = $scope.first.clone();
	        results = [];
	        while (true) {
	          $scope.days[day.format('YYYY-MM-DD')] = {
	            m: day.clone(),
	            selected: false,
	            events: new Array(),
	            coverEvent: {},
	      ...
	      $scope.reloadEvents = function() {
	        return replicas.request(_.defaults({
	          starts: $scope.first.clone().toDate(),
	          ends: $scope.last.clone().toDate()
	        }, $scope._filters)).then(function(result) {
	          taxonomies.translate($scope._filters.taxonomies);
	          $scope.events = result;
	          _.forEach($scope.events, function(event) {
	            var day, results;
	            if (!_.some(event.event.taxonomies, {
	              'name': 'holiday'
	            })) {
	              day = moment.max(moment(event.starts), $scope.first).clone();
	              results = [];
	              while (true) {
	                if (day.isAfter(moment.min(event.ends, $scope.last))) {
	                  break;
	                }
	                $scope.days[day.format('YYYY-MM-DD')].events.push(event);
	          ...
	          _.forEach($scope.days, function(day) {
	            day.coverEvent = $scope._getRandomEvent(day.events);
	            return $scope.visible_days[day.m.format('YYYY-MM-DD')] = day;
	          });
	          $scope.days.length = 0;
	          return $scope.reloadHolidays();
	        });
	      };
	      $scope._getRandomEvent = function(events) {
	        if (events) {
	          return events[_.random(15) % (events != null ? events.length : void 0)];
	        }
	      };
	      $scope.reloadHolidays = function() {
	        return replicas.request(_.defaults({
	          starts: $scope.first.clone().toDate(),
	          ends: $scope.last.clone().toDate(),
	          taxonomies: {
	          ...
	          return _.forEach(result, function(event) {
	            var day, ref;
	            day = moment.max(moment(event.starts), $scope.first).clone();
	            return (ref = $scope.visible_days[day.format('YYYY-MM-DD')]) != null ? ref.holidays.push(event) : void 0;
	          });
	        });
	      };
	      $scope._getCalendarStarts = function(date) {
	        if (date) {
	          return $scope.current = moment(date, 'YYYY-MM-DD');
	        } else {
	          $scope.current = moment();
	          if ($scope.current.date() > 10) {
	            return $scope.current.add(1, 'months');
	          }
	        }
	      };
	      $scope.reload = function() {
	        $scope.close_details();
	        $scope.visible_days.length = 0;
	        $scope.createCalendar();
	        return $scope.reloadEvents();
	      };
	      $scope.init();
	      return $scope.reload();
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	'use strict';
	angular.module('smartvel-calendar').directive('smtDay', function() {
	  return {
	    ...
	    controller: function($scope) {
	      $scope.has_events = function(day) {
	        return _.size(day.events) > 0;
	      };
	      return $scope.is_relevant = function(day) {
	        return $scope.has_events(day);
	      };
	    }
	  };
	});
...
/***/ function(module, exports) {
	...
	module.exports = "<div ng-class=\"{'empty-day': !is_relevant(day)}\"\n     class=\"smartimg\"\n     data-pictures=\"{{ day.coverEvent.event.pictures }}\">\n\n    <div class=\"enabled-day-overlay\"\n         ng-class=\"{'selected-day': day.selected}\">\n\n        <div class=\"header\">\n            <div class=\"date\">\n                <div class=\"month-day\">\n                    {{ day.m.format('D') }}\n                </div>\n                <div class=\"weekday\">\n                    {{ day.m.format('dddd') }}\n                </div>\n            </div>\n            <div class=\"holiday\"> {{ day.holidays[0].event.name }} </div>\n        </div>\n\n        <div class=\"main_event\">\n            <p class=\"event_taxonomy\">{{ day.coverEvent.event.taxonomies[0].name | capitalize_all }}</p>\n\n            <p class=\"event_name\">{{ day.coverEvent.event.name | capitalize_all }}</p>\n        </div>\n\n        <div class=\"summary\">\n          <span class=\"total_events\">+{{ day.events.length }} {{'CUST_EVENTS' | translate}} </span>\n        </div>\n\n    </div>\n\n    <div ng-hide=\"is_relevant(day)\"\n         class=\"disabled-day-overlay\"\n         ng-class=\"{'selected-day': day.selected}\">\n\n        <div class=\"date\">\n            <p class=\"day\">{{ day.m.format('D') }}</p>\n        </div>\n\n    </div>\n\n</div>\n";
...
/***/ function(module, exports, __webpack_require__) {
	...
	'use strict';
	angular.module('smartvel-calendar').directive('smtEventDayList', function() {
	  return {
...
/***/ function(module, exports) {
	...
	module.exports = "<div class=\"events-day-list\"\n     ng-class=\"{'expanded': details_displayed, 'collapsed': !details_displayed}\"\n     id=\"calendar-details\">\n    <div class=\"actions custom-pull-right\">\n        <span class=\"glyphicon glyphicon-menu-up\" ng-click=\"close_details()\"></span>\n    </div>\n    <div class=\"events\">\n        <div ng-repeat=\"item in events\" class=\"list-element\">\n          <smt-event type=\"list\"\n                     event=\"item.event\"\n                     day=\"selected_day\"\n                     allow-travelplan=\"=\"\n                     book-params=\"=\"\n                     allow-tickets=\"=\">\n          </smt-event>\n        </div>\n    </div>\n</div>\n";

/***/ },
/* 90 */
/***/ function(module, exports) {

	module.exports = "<div class=\"smt-calendar events\">\n\n    <smt-searchbox ng-show=\"_params.searchBox != undefined\"\n                   filters=\"_filters\"\n                   custom-controls=\"['taxonomies', 'regions', 'text']\"\n                   reload=\"reload()\">\n    </smt-searchbox>\n\n    <div class=\"month-selector\">\n        <p>\n            <span class=\"custom-pull-left\" ng-click=\"prev_month()\">&lt;&lt;</span>\n            {{ current.format('YYYY-MMMM') }}\n            <span class=\"custom-pull-right\" ng-click=\"next_month()\">&gt;&gt;</span>\n        </p>\n    </div>\n\n    <div class=\"days-panel\">\n\n      <div ng-repeat=\"daylabel in week_days\" class=\"weekday\">\n        {{ daylabel | uppercase }}\n      </div>\n\n      <div ng-repeat=\"day in visible_days\" class=\"calendar-day\"\n           ng-attr-id=\"{{ 'day-'+day.m.format('YYYY-DD-MM') }}\"\n           ng-class=\"{ 'past-day': isDisabledDay(day),\n                       'out-of-month': !isInCurrentMonth(day) }\"\n           ng-click=\"show_events(day)\">\n\n         <div class=\"calendar-day-placeholder\" ng-show=\"isInCurrentMonth(day)\">\n             <smt-day day=\"day\"></smt-day>\n         </div>\n\n       </div>\n\n    </div>\n</div>\n";
...
/***/ function(module, exports, __webpack_require__) {
	...
	angular.module('smartvel-inspire', ['smartvel-common', 'smartvel-calendar', 'smartvel-map', 'smartvel-highlights']);
...
/***/ function(module, exports, __webpack_require__) {
	...
	/**
	  * @ngdoc directive
	  * @name smartvel.directive:smtInspire
	  * @description
	  * # smtInspire
	 */
	angular.module('smartvel-inspire').directive('smtInspire', function(Session) {
	  return {
	    ...
	    controller: function($scope, $attrs, params) {
	      $scope._params = params.getParams($attrs);
	      $scope.view = 'map';
	      $scope.changeView = function(newView) {
	        return $scope.view = newView;
	      };
	      return $scope.checkView = function(view) {
...
/***/ function(module, exports) {
	...
	module.exports = "<ul class=\"nav nav-tabs\">\n        <li class=\"active\"><a data-toggle=\"tab\" ng-click=\"changeView('map')\">Map</a></li>\n        <li><a data-toggle=\"tab\" ng-click=\"changeView('calendar')\">Calendar</a></li>\n        <li><a data-toggle=\"tab\" ng-click=\"changeView('highlights')\">Highlights</a></li>\n</ul>\n    <div class=\"tab-content\">\n        <div class=\"tab-pane fade in active\" ng-show=\"checkView('highlights')\">\n\n            <smt-highlights search-box=\"=\">\n                <h1>\n                  {{'INSPIRE_TEXT' | translate}}\n                </h1>\n            </smt-highlights>\n        </div>\n        <div class=\"tab-pane fade in active\" ng-show=\"checkView('map')\">\n            <smt-map\n                    search-box=\"=\"\n                    latitude=\"=\"\n                    longitude=\"=\"\n                    zoom=\"=\"\n                    allow-tickets=\"=\"\n                    show-places=\"=\"\n                    allow-travelplan=\"=\"\n                    book-params=\"=\"\n                    >\n            </smt-map>\n        </div>\n        <div class=\"tab-pane fade in active\" ng-show=\"checkView('calendar')\">\n              <smt-calendar\n                  search-box=\"=\"\n                  allow-tickets=\"=\"\n                  allow-travelplan=\"=\"\n                  book-params=\"=\"\n                    >\n              </smt-calendar>\n        </div>\n</div>\n";
